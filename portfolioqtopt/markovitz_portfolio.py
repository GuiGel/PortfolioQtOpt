...
class Selection:
    def __init__(self, prices: npt.NDArray[np.floating[typing.Any]], w: int, budget: float) -> None:
    ...
    def _get_expand_prices(self, reversed: bool = False) -> npt.NDArray[np.floating[typing.Any]]:
        ...
        normalized_prices = np.divide(self.b, factor, dtype=np.float64, casting="unsafe")
        all_assert_prices = self.prices[:, :, np.newaxis] * self.granularity * normalized_prices.reshape(-1, 1)
