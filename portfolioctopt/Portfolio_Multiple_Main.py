...
def interpretarResultados(fondos, slices, dwave_raw_array, portfolio_selection, new_header, price_data, lista_fondos_invertidos=[]):
    inversiones = []
    ...
    final = dwave_raw_array[0] * portfolio_selection.price_data_reversed[
    ...
    portfolio_selection.price_data_reversed = portfolio_selection.price_data_reversed * 100

    for i in range(len(inversiones)):
        desviaciones = desviaciones + (pow(inversiones[i], 2) * pow(numpy.std(price_data[:, i]), 2))
    ...
    for i in range(len(inversiones)):
        for j in range(len(inversiones)):
            if i != j:
                covarianzas = covarianzas + (inversiones[i] * inversiones[j] * (
                numpy.cov(pd.to_numeric(price_data[:, i]), pd.to_numeric(price_data[:, j]))[0][1]))

    #print("Risk: ", str(numpy.sqrt(desviaciones + covarianzas)))

    #print("----------------------------")

    return lista_fondos_invertidos, inversiones_unicas, cantidades_unicas, str(100 * numpy.sum(retornos)), str(numpy.sqrt(desviaciones + covarianzas)), (100 * numpy.sum(retornos))/(numpy.sqrt(desviaciones + covarianzas))
...
def main():
    ...
    for i in range(repetitions):
        ...
        dwave_raw_array, portfolio_selection, new_header, price_data = Portfolio_Calculation.Portfolio_Calculation(API,slices,file_name,sheet,fondos)

        lista_fondos_invertidos = interpretarResultados(fondos, slices, dwave_raw_array, portfolio_selection, new_header, price_data, lista_fondos_invertidos)[0]
    ...
    for i in range(repetitions):
        dwave_raw_array, portfolio_selection, new_header, price_data = Portfolio_Calculation.Portfolio_Calculation(API,slices,file_name,sheet,len(lista_fondos_invertidos),lista_fondos_invertidos=lista_fondos_invertidos)
        aux_fondos_finales, aux_inversiones_unicas, aux_inversiones, aux_return, aux_risk, aux_SHARPE = interpretarResultados(len(lista_fondos_invertidos), slices, dwave_raw_array,portfolio_selection, new_header, price_data)
