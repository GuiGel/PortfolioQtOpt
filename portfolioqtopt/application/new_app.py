...
def step_0_form() -> Tuple[bool, Optional[UploadedFile], str]:
    with st.sidebar:
        st.markdown(f"## Parameters for loading historical assets")
        with st.form(key="xlsm"):
            with st.expander("Load from xlsm"):
...
def step_1_form(
    ...
    with st.sidebar:
        st.markdown("## Parameters for assets simulation")
        with st.form(key="simulation"):
            with st.expander(
                ...
                days_number = st.number_input(
                    "numero de días a simular", value=256, step=1
                )
...
def step_2_form() -> Tuple[bool, float, int, float, float, float, int]:
    ...
    with st.sidebar:
        st.markdown("## Parameters for portfolio optimization")
        ...
        if sub1 or memory.get("step_2_form"):
            ...
            if not sub0:
                ...
                if sub2:
                    ...
                    st.markdown("## Find the best portfolio")
